...
// Interface que define a estrutura exata das suas classes
...
function AboutSection() {
  // O Record<string, CharacterClass> garante que cada chave do objeto obedeça à interface
  const charactersClasses: Record<string, CharacterClass> = {
    developerClass: {
  ...
  const classesKeys = useMemo(() => Object.keys(charactersClasses), [charactersClasses]);
  const [currentClassIndex, setCurrentClass] = useState<number>(0);
      ...
      <div className="about-section-cards">
        ...
        >
          ...
          <div className="right-card-bottom">
            {actualClass.classSkills.map((skill: string, index: number) => (
